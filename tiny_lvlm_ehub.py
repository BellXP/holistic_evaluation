...
def main(args):
    ...
    root_dir = 'datasets/tiny_lvlm_ehub_random_50samples/samples_jsonl'
    # result = {}
    # if args.eval_ocr:
    # OCR_datasets
    task = 'OCR'
    ...
    task_datasets = {
        ...
        'Caption': ['Flickr', 'NoCaps', 'WHOOPSCaption', 'MSCOCO_caption_karpathy'],
        'KIE': ['SROIE', 'FUNSD', 'POIE'],
        'VQA': [
            'DocVQA', 'GQA', 'IconQA', 'OCRVQA', 'STVQA', 'TextVQA', 'Visdial',
            'VSR', 'OKVQA', 'WHOOPSVQA', 'VizWiz', 'WHOOPSWeird', 'VQAv2',
            'AOKVQAOpen', 'AOKVQAClose', 'HatefulMemes', 'ScienceQAIMG',
            'ImageNetVC_shape', 'ImageNetVC_color', 'ImageNetVC_component',
            'ImageNetVC_material', 'ImageNetVC_others', 'RSVQALR_mci', 'COD10K'
        ],
        ...
        for dataset_name in task_datasets[task]:
            ...
            with open(jsonl_path, 'w') as f:
                for x in dataset:
                    f.write(json.dumps(x) + '\n')
    cifar = {
        'CIFAR10': 'datasets/cifar-10-batches-py/images',
