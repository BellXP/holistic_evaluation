...
class MetaModel(nn.Module):
    ...
    """
    def __init__(self, llama_type, llama_config, tokenizer_path, reversible_grad=False,
                 with_visual=True, max_seq_len=2048, max_batch_size=32):
        super().__init__()
    ...
    def forward(self, examples, labels, images):
    ...
    def generate(
        ...
        stop_generating = [False for _ in range(bsz)]
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            for i in range(bsz):
                if next_token[i] == self.tokenizer.eos_id:
                    stop_generating[i] = True
            if all(stop_generating):
                break
            prev_pos = cur_pos
