...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        base_dir = 'checkpoints/llama_checkpoints/'
        tokenizer_path = base_dir + 'tokenizer.model'
        llama_7b_dir = base_dir + '7B'
...
###        adapter_weight = torch.load('llama_adapter_len10_layer30_release.pth', map_location="cpu")
        ...
        # TBD access hyperparameters vis environment variable
        ...
        for wnid in mappings:
            for label in mappings[wnid]:
                token_ids = mappings[wnid][label]['ids']
                image1k_token_ids.extend(token_ids)
        eos_id = self.tokenizer.eos_id
        image1k_token_ids.append(eos_id)
        self.image1k_token_ids = sorted(list(set(image1k_token_ids)))
        decoding_mask = torch.ones(self.tokenizer.n_words, dtype=torch.bool)
        decoding_mask[self.image1k_token_ids] = False
        self.decoding_mask = decoding_mask
        enable_decoding_mask = os.environ.get('enable_decoding_mask', False)
        self.enable_decoding_mask = True if enable_decoding_mask else False
        print(f'enable decoding mask = {self.enable_decoding_mask}')
...
#        model.load_state_dict(adapter_weight, strict=False)
    ...
    def generate(
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            if self.enable_decoding_mask:
                logits[:, self.decoding_mask] = float('-inf')
            if temperature > 0:
