...
class LLaMA_adapter(nn.Module):
    ...
    max_seq_len=512, max_batch_size=1):
        ...
        model_args: ModelArgs = ModelArgs(
            max_seq_len=max_seq_len, max_batch_size=max_batch_size, w_bias=bias_lora, w_lora=bias_lora,  **params
    ...
    def forward_visual(self, inputs, cache_size=10, cache_t=20, cache_weight=0.5):
        ...
        if self.knn:
            ...
            visual_feats = (sims[:, :, None] * prototypes).sum(dim=1)
...
def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts',
         knn=False, llama_type="7B", phase="finetune", max_seq_len=512, max_batch_size=1):
    if name in _MODELS:
