...
dataset2task = {}
for x in Path('datasets/tiny_lvlm_ehub_random_50samples/bard_evaluation').glob('*.jsonl'):
    y = x.name.split('_')
    task = y[0]
    dataset = '_'.join(y[1:-2])
    if dataset == 'RSVQALR_mci':
        dataset = 'RSVQALR_MCI'
    dataset2task[dataset] = task

bard_evaluation = root_dir / 'tiny_answers_0801/Bard'

jsonls = sorted(list(bard_evaluation.glob('*.json')))
...
df = df.sort_values(['Task', 'Dataset'])
print(df)
